...
type Config struct {
	...
	// SocketReplacedTime 顶号后老连接的存活期，单位秒：收到顶号请求后它还能活多久
	// （期间只收不发），倒计时结束才断开。新端是立刻接管还是等它下线由上层策略决定，
	// 与这个时长无关——两种策略下老连接都是这一套处置。
	// ⚠️ 应 <= SocketConnectTime：倒计时复用的就是心跳超时那个计数器，
	// 而 Close 只缩短不延长剩余存活时间，配得再大也不会超过 SocketConnectTime。
	// 想要更长的存活期，必须把 SocketConnectTime 一起调大。
...
var Options = Config{
	...
	SocketReplacedTime:      30,     // 顶号后老连接存活 30 秒（= SocketConnectTime 上限）
