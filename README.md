...
| `EventTypeReplaced`       | 被顶号（进入存活期） | `*Replaced`（顶号方 IP + 剩余存活秒数） |
...
cosnet.Options = cosnet.Config{
    ...
    SocketReplacedTime:      30,     // 顶号后老连接存活期（秒），须 <= SocketConnectTime
...
- `sock.Replaced(newIP)` **通知顶号**（不是立即踢人）：老连接进入 `SocketReplacedTime` 秒的存活期，
  期间 **只收不发**——服务器推送和在途回包照常送达，它自己发来的新请求一律被拒；倒计时结束才断开。
  已在存活期内重复调用返回 `false`，不重复通知也不重置倒计时。
  新端是立刻接管还是等它下线，由上层决定（网关侧是 `gate.forceReplace`），本层不做取舍。
